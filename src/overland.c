...
{
	"sangang.raw", "nyemo.raw", "jaili.raw",
	"yakongza.raw", "xianza.raw", "bangoin.raw",
	"konjo.raw", "zogang.raw", "tangmai.raw"
};

char *const map_names[] =
{
	"sangyang", "nyemo", "jaili",
	"yakongza", "xianza", "bangoin",
	"konjo", "zogang", "tangmai"
};
...
char *const sect_types[] =
{
	...
	"trail", "blands", "grassland", "scrub", "barren", "bridge",
	"road",
	"\n"
};
...
const struct sect_color_type sect_show[] =
{
	...
	{SECT_CITY, 		"&Y", "\xCD", 	"city", 	TRUE, 1, 255, 128, 54},
	{SECT_FIELD, 		"&G", "\xB0", 	"field", 	TRUE, 1, 141, 215, 1},
	{SECT_FOREST, 	"&g", "\xAF", 	"forest", 	TRUE, 1, 1, 108, 47},
	{SECT_HILLS, 		"&O", "\xA6", 	"hills", 	TRUE, 1, 140, 102, 57},
	{SECT_MOUNTAIN, 	"&w", "\xA2", 	"mountain", 	TRUE, 1, 152, 152, 152},
	{SECT_WATER_SWIM, 	"&C", "\xA9", 	"shallow water",TRUE, 1, 102, 235, 242},
	{SECT_WATER_NOSWIM, 	"&B", "\xAA", 	"deep water", 	TRUE, 1, 67, 115, 251},
	{SECT_AIR, 		"&x", "?", 	"air",		FALSE, 1, 0, 0, 0},
	{SECT_UNDERWATER, 	"&x", "?", 	"underwater", 	FALSE, 1, 0, 0, 0},
	{SECT_DESERT, 	"&Y", "\xB4", 	"desert",	TRUE, 1, 241, 228, 145},
	{SECT_RIVER, 		"&B", "\xAB", 	"river", 	TRUE, 1, 0, 0, 255},
	{SECT_OCEANFLOOR, 	"&x", "?", 	"ocean floor",	FALSE, 1, 0, 0, 0},
	{SECT_UNDERGROUND, 	"&x", "?", 	"underground", 	FALSE, 1, 0, 0, 0},
	{SECT_JUNGLE, 	"&g", "*", 	"jungle", 	TRUE, 1, 70, 149, 52},
	{SECT_SWAMP, 		"&g", "\xB4", 	"swamp", 	TRUE, 1, 218, 176, 56},
	{SECT_TUNDRA, 	"&C", "-", 	"tundra", 	TRUE, 1, 54, 255, 255},
	{SECT_ICE, 		"&W", "\xA4", 	"ice", 		TRUE, 1, 133, 177, 252},
	{SECT_OCEAN, 		"&b", "~", 	"ocean", 	FALSE, 1, 0, 0, 128},
	{SECT_LAVA, 		"&R", ":", 	"lava", 	FALSE, 1, 245, 37, 29},
	{SECT_SHORE, 		"&Y", "\xA8", 	"shoreline", 	TRUE, 1, 221, 255, 38},
	{SECT_TREE, 		"&g", "\xAE", 	"impass forest",FALSE, 1, 0, 64, 0},
	{SECT_STONE, 		"&W", "\xA3", 	"impass mountain", FALSE, 1, 128, 128, 128},
	{SECT_QUICKSAND, 	"&g", "%", 	"quicksand", 	FALSE, 1, 128, 128, 0},
	{SECT_WALL, 		"&P", "\xCC", 	"wall", 	FALSE, 1, 255, 0, 255},
	{SECT_GLACIER, 	"&W", "\xA5", 	"glacier", 	FALSE, 1, 141, 207, 244},
	{SECT_EXIT, 		"&W", "\xB7", 	"exit", 	TRUE, 1, 255, 255, 255},
	{SECT_TRAIL, 		"&O", ":", 	"trail", 	TRUE, 1, 128, 64, 0},
	{SECT_BLANDS, 	"&r", ".", 	"blasted lands",TRUE, 1, 128, 0, 0},
	{SECT_GRASSLAND, 	"&G", "\xB2", 	"grassland", 	TRUE, 1, 83, 202, 2},
	{SECT_SCRUB, 		"&g", "\xB3", 	"scrub", 	TRUE, 1, 123, 197, 112},
	{SECT_BARREN, 	"&O", ".", 	"barren", 	TRUE, 1, 192, 192, 192},
	{SECT_BRIDGE, 	"&P", "\xCF", 	"bridge", 	TRUE, 1, 255, 0, 128},
...

	{
...

		{
...

			{
				...
				ch_printf ( ch, "Distance to landmark: %f\r\n", dist );
...
void do_landmarks ( CHAR_DATA * ch, char *argument )
{
...

	{
		...
		pager_printf ( ch, "%-10s  %-4dX %-4dY   %-4d       %s\r\n",
		               map_names[landmark->map], landmark->x, landmark->y,
		               landmark->distance, landmark->description );
...
void do_setmark ( CHAR_DATA * ch, char *argument )
{
	...
	if ( arg[0] == STRING_NULL || !str_cmp ( arg, "help" ) )
...
void load_entrances ( void )
{
	...
	log_string ( " Done Overland Exits" );
	return;
}
...
void do_setexit ( CHAR_DATA * ch, char *argument )
{
...

	{
		...
		if ( !str_cmp ( arg2, "sangang" ) )

			map = ACON_C1;

		else

			if ( !str_cmp ( arg2, "nyemo" ) )

				map = ACON_C2;

			else

				if ( !str_cmp ( arg2, "jaili" ) )

					map = ACON_C3;

				else

					if ( !str_cmp ( arg2, "yakongza" ) )

						map = ACON_C4;

					else

						if ( !str_cmp ( arg2, "xianza" ) )

							map = ACON_C5;

						else

							if ( !str_cmp ( arg2, "bangoin" ) )

								map = ACON_C6;

							else

								if ( !str_cmp ( arg2, "konjo" ) )

									map = ACON_C7;

								else

									if ( !str_cmp ( arg2, "zogang" ) )

										map = ACON_C8;

									else

										if ( !str_cmp ( arg2, "tangmai" ) )
...
void mapreset ( void )
{
	...
	for ( reset = first_mapreset; reset; reset = reset->next )
	{
		...
		{
			case MAP_C1:
				toroom = get_room_index ( OVERLAND_MAP_SANGANG );
				break;
			case MAP_C2:
				toroom = get_room_index ( OVERLAND_MAP_NYEMO );
				break;
			case MAP_C3:
				toroom = get_room_index ( OVERLAND_MAP_JAILI );
				break;
			case MAP_C4:
				toroom = get_room_index ( OVERLAND_MAP_YAKONGZA );
				break;
			case MAP_C5:
				toroom = get_room_index ( OVERLAND_MAP_XIANZA );
				break;
			case MAP_C6:
				toroom = get_room_index ( OVERLAND_MAP_BANGOIN );
				break;
			case MAP_C7:
				toroom = get_room_index ( OVERLAND_MAP_KONJO );
				break;
			case MAP_C8:
				toroom = get_room_index ( OVERLAND_MAP_ZOGANG );
				break;
			case MAP_C9:
				toroom = get_room_index ( OVERLAND_MAP_TANGMAI );
				break;
			default:
...
void do_mapresets ( CHAR_DATA * ch, char *argument )
{
	...
	for ( reset = first_mapreset; reset; reset = reset->next )
	{
		...
		pager_printf ( ch, "%-7.7s %5d   %-25.25s %-10.10s %4dX %4dY\r\n", reset->type == TYPE_OBJECT ? "Object" : "Mobile",
		               reset->vnum, log_buf, map_names[reset->map],
		               reset->x, reset->y );
	}
	return;
}
...
void do_mreset ( CHAR_DATA * ch, char *argument )
{
	...
	if ( arg[0] == STRING_NULL || ( str_cmp ( arg, "add" ) && str_cmp ( arg, "delete" ) ) )
	{
	...
	if ( arg2[0] == STRING_NULL || ( str_cmp ( arg2, "object" ) && str_cmp ( arg2, "mobile" ) ) )
	{
		do_mreset ( ch, "" );
		return;
	}
...
void new_map_to_char ( CHAR_DATA * ch, short startx, short starty, short endx, short endy, int radius )
{
	...
	for ( y = starty; y < endy + 1; y++ )
	{
		for ( x = startx; x < endx + 1; x++ )
		{
			...
			if ( object && !other )
				strncat ( secbuf, "&Y\xBD", MAX_STRING_LENGTH );

			if ( other )
			{
				if ( npc )
					strncat ( secbuf, "&C\xBC", MAX_STRING_LENGTH ); // TODO mobile colours based on level
				else
					strncat ( secbuf, "&P\xBA", MAX_STRING_LENGTH );
			}

			if ( x == ch->x && y == ch->y )
			{
				if ( group )
					strncat ( secbuf, "&Y\xBE", MAX_STRING_LENGTH );
				else
					strncat ( secbuf, "&R\xBE", MAX_STRING_LENGTH );
...
void display_map ( CHAR_DATA * ch )
{
	...
	if ( ch->map == -1 )
	{
		...
		enter_map ( ch, 50, 50, ACON_C1 );
		return;
	}
	sector = get_terrain ( ch->map, ch->x, ch->y );
	send_to_char ( "\r\n", ch );

	if ( ch->in_room->vnum == 501 )
	{
		ch_printf ( ch, "&z-{  &rPK ROOM  &z}- &RThe Region of Sangang.                       &cCoords: &W%d&cx &W%d&cy\r\n", ch->x, ch->y );
	}
	else if ( ch->in_room->vnum == 502 )
	{
		ch_printf ( ch, "&z-{  &rPK ROOM  &z}- &RThe Region of Nyemo.			     &cCoords: &W%d&cx &W%d&cy\r\n", ch->x, ch->y );
	}
	else if ( ch->in_room->vnum == 503 )
	{
		ch_printf ( ch, "&z-{  &rPK ROOM  &z}- &RThe Region of Jaili			     &cCoords: &W%d&cx &W%d&cy\r\n", ch->x, ch->y );
	}
	else if ( ch->in_room->vnum == 504 )
	{
		ch_printf ( ch, "&z-{  &rPK ROOM  &z}- &RThe Region of Yakongza 			     &cCoords: &W%d&cx &W%d&cy\r\n", ch->x, ch->y );
	}
	else if ( ch->in_room->vnum == 505 )
	{
		ch_printf ( ch, "&z-{  &rPK ROOM  &z}- &RThe Region of Xianza   			     &cCoords: &W%d&cx &W%d&cy\r\n", ch->x, ch->y );
	}
	else if ( ch->in_room->vnum == 506 )
	{
		ch_printf ( ch, "&z-{  &rPK ROOM  &z}- &RThe Region of Bangoin                        &cCoords: &W%d&cx &W%d&cy\r\n", ch->x, ch->y );
	}
	else if ( ch->in_room->vnum == 507 )
	{
		ch_printf ( ch, "&z-{  &rPK ROOM  &z}- &RThe Region of Konjo                          &cCoords: &W%d&cx &W%d&cy\r\n", ch->x, ch->y );
	}
	else if ( ch->in_room->vnum == 508 )
	{
		ch_printf ( ch, "&z-{  &rPK ROOM  &z}- &RThe Region of Zogang                         &cCoords: &W%d&cx &W%d&cy\r\n", ch->x, ch->y );
	}
	else if ( ch->in_room->vnum == 509 )
	{
		ch_printf ( ch, "&z-{  &rPK ROOM  &z}- &RThe Region of Tangmai                        &cCoords: &W%d&cx &W%d&cy\r\n", ch->x, ch->y );
	}
	...
	new_map_to_char ( ch, startx, starty, endx, endy, 100 );
	landmark = check_landmark ( ch->map, ch->x, ch->y );
	char temp[MAX_STRING_LENGTH];
	char  buf[MAX_STRING_LENGTH];
	strcpy ( buf, random_description ( ch, temp ) );
	send_to_char ( "&g--------------------------------------------------------------------------------\r\n", ch );
	ch_printf ( ch, "&w%s%s\r\n", impass_message[sector], buf );
	send_to_char ( "&g--------------------------------------------------------------------------------\r\n", ch );
	send_mip_exits ( ch );

	if ( landmark && landmark->Isdesc )
		ch_printf ( ch, "&G%s %s", landmark->description ? landmark->description : "", buf );

	if ( IS_IMMORTAL ( ch ) )
	{
		ch_printf ( ch, "\r\n&GSector type: %s. Coordinates: %dX, %dY\r\n", sect_types[sector], ch->x, ch->y );
		...
		if ( reset )
		{
			...
			{
				if ( reset->map == ch->map && reset->x == ch->x && reset->y == ch->y )
					ch_printf ( ch, "&P%s reset present: Vnum %d\r\n", reset->type == TYPE_OBJECT ? "Object" : "Mobile",  reset->vnum );
			}
		}
...
bool map_wander ( CHAR_DATA * ch, int map, int x, int y, int sector )
{
	 ...
	 * t will only wander in that type of terrain - Samson 7-29-00
	 */
...
ROOM_INDEX_DATA * find_continent ( CHAR_DATA * ch, ROOM_INDEX_DATA * maproom )
{
...

	{
		...
		location = get_room_index ( OVERLAND_MAP_SANGANG );
...

	{
		...
		location = get_room_index ( OVERLAND_MAP_NYEMO );
...

	{
		...
		location = get_room_index ( OVERLAND_MAP_JAILI );
	...
	if ( maproom->area->continent == ACON_C4 )

	{

		location = get_room_index ( OVERLAND_MAP_YAKONGZA );
	...
	if ( maproom->area->continent == ACON_C5 )

	{

		location = get_room_index ( OVERLAND_MAP_XIANZA );
	...
	if ( maproom->area->continent == ACON_C6 )

	{

		location = get_room_index ( OVERLAND_MAP_BANGOIN );
	...
	if ( maproom->area->continent == ACON_C7 )

	{

		location = get_room_index ( OVERLAND_MAP_KONJO );
	...
	if ( maproom->area->continent == ACON_C8 )

	{

		location = get_room_index ( OVERLAND_MAP_ZOGANG );
	...
	if ( maproom->area->continent == ACON_C9 )

	{

		location = get_room_index ( OVERLAND_MAP_TANGMAI );
...

	{
...

		{
			...
			case ACON_C1:
				...
				maproom = get_room_index ( OVERLAND_MAP_SANGANG );
			...
			case ACON_C2:
				...
				maproom = get_room_index ( OVERLAND_MAP_NYEMO );
			...
			case ACON_C3:
				...
				maproom = get_room_index ( OVERLAND_MAP_JAILI );
			...
			case ACON_C4:
				...
				maproom = get_room_index ( OVERLAND_MAP_YAKONGZA );
			...
			case ACON_C5:
				...
				maproom = get_room_index ( OVERLAND_MAP_XIANZA );
			...
			case ACON_C6:
				...
				maproom = get_room_index ( OVERLAND_MAP_BANGOIN );
			...
			case ACON_C7:
				...
				maproom = get_room_index ( OVERLAND_MAP_KONJO );
			...
			case ACON_C8:
				...
				maproom = get_room_index ( OVERLAND_MAP_ZOGANG );
			...
			case ACON_C9:
				...
				maproom = get_room_index ( OVERLAND_MAP_TANGMAI );
...
/* Added by Samson 12-10-97. Takes sector name being input and returns the
   numerical value back. Called by do_rset when editing sector types.
