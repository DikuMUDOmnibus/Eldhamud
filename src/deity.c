...
void save_deity( DEITY_DATA * deity )
{
   ...
   if( !deity->filename || deity->filename[0] == STRING_NULL )
   {
...
void do_showdeity( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf( ch, "Race: %-12sClass: %-11sSex: %-13sRace2: %s\r\n",
              ...
              deity->sex == SEX_FEMALE ? "female" : "neutral", ( deity->race2 < 0 || deity->race2 > MAX_PC_RACE ) ? "none" : npc_race[deity->race2] );
...
void do_deities( CHAR_DATA * ch, char *argument )
{
   DEITY_DATA *deity;
   int count = 0;
   set_char_color( AT_DEITY, ch );
   if( argument[0] == STRING_NULL )
   {
      send_to_pager_color( "\r\nFor detailed information on a deity, try 'deities <deity>' or 'help deities'\r\n", ch );
      send_to_char( "--------------------------------------------------------------------------------\r\n", ch );
      send_to_pager_color( "&c Deity			Worshippers&D\r\n", ch );
      send_to_char( "--------------------------------------------------------------------------------\r\n", ch );
      for( deity = first_deity; deity; deity = deity->next )
      {
         pager_printf_color( ch, " &G%-14s	&w%d&D\r\n", deity->name, deity->worshippers );
         count++;
      }
      if( !count )
      {
         send_to_pager_color( "&DThere are no deities on this world.\r\n", ch );
      }
      send_to_char( "&D--------------------------------------------------------------------------------\r\n", ch );
      return;
   }
   ...
   if( !deity )
   {
      send_to_pager_color( "&DThat deity does not exist.\r\n", ch );
      return;
   }
   send_to_char( "--------------------------------------------------------------------------------\r\n", ch );
   pager_printf_color( ch, "&cDeity:        &d%s&D\r\n", deity->name );
   send_to_char( "--------------------------------------------------------------------------------\r\n", ch );
   pager_printf_color( ch, "&cDescription:\r\n&w%s&D", deity->description );
   send_to_char( "&D--------------------------------------------------------------------------------\r\n", ch );
   return;
}
   ...
   if( !str_cmp( arg, "corpse" ) && !IS_PKILL( ch ) )
   {
      ...
      for( obj = first_object; obj; obj = obj->next )
      {
         if( obj->in_room && !str_cmp( buf2, obj->short_descr ) && ( obj->pIndexData->vnum == 11 ) )
         {
            ...
            obj_from_room( obj );
            obj = obj_to_room( obj, ch->in_room, supermob );
         }
      }
      if( !found )
      {
