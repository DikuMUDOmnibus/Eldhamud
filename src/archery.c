...
void do_dislodge ( CHAR_DATA * ch, char *argument )
{
	...
	if ( argument[0] == STRING_NULL )  /* empty */
 ...
 */
ch_ret ranged_got_target ( CHAR_DATA * ch, CHAR_DATA * victim, OBJ_DATA * weapon, OBJ_DATA * projectile, short dist,
...
ch_ret ranged_attack ( CHAR_DATA * ch, char *argument, OBJ_DATA * weapon, OBJ_DATA * projectile, short dt, short range )
{
	...
	if ( argument && argument[0] != STRING_NULL && argument[0] == '\'' )
	{
...

	{
		...
		if ( skill->noun_damage && skill->noun_damage[0] != STRING_NULL )
...

	{
...

		{
			...
			if ( projectile )
				...
				snprintf ( buf, MAX_STRING_LENGTH, "You see your %s hit a door in the distance to the %s.", stxt,
...

		{
...

			{
				...
				act ( color, "Your $t falls harmlessly to the ground to the $T.",
				      ch, myobj ( projectile ), dir_name[dir],
...
void do_fire ( CHAR_DATA * ch, char *argument )
{
	...
	if ( arg[0] == STRING_NULL && ch->fighting == NULL )
