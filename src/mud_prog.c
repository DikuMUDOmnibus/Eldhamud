...
int mprog_do_ifcheck args( ( char *ifcheck, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, CHAR_DATA * rndm ) );
...
bool mprog_keyword_check( const char *argu, const char *argl )
{
   ...
   else
   {
      ...
      for( ; word[0] != STRING_NULL; arglist = one_argument( arglist, word ) )
   ...
   for( mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next )
      if( mprg->type == type )
      {
         ...
         else
         {
            ...
            for( ; word[0] != STRING_NULL; list = one_argument( list, word ) )
...
void mprog_script_trigger( CHAR_DATA * mob )
{
   ...
   if( HAS_PROG( mob->pIndexData, SCRIPT_PROG ) )
      for( mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next )
         if( mprg->type == SCRIPT_PROG && ( mprg->arglist[0] == STRING_NULL || mob->mpscriptpos != 0 || atoi( mprg->arglist ) == time_info.hour ) )
...
void oprog_script_trigger( OBJ_DATA * obj )
{
   ...
   if( HAS_PROG( obj->pIndexData, SCRIPT_PROG ) )
      for( mprg = obj->pIndexData->mudprogs; mprg; mprg = mprg->next )
         ...
         {
            if( mprg->arglist[0] == STRING_NULL || obj->mpscriptpos != 0 || atoi( mprg->arglist ) == time_info.hour )
            {
               set_supermob( obj );
...
void rprog_script_trigger( ROOM_INDEX_DATA * room )
{
   ...
   if( HAS_PROG( room, SCRIPT_PROG ) )
      for( mprg = room->mudprogs; mprg; mprg = mprg->next )
         if( mprg->type == SCRIPT_PROG )
         {
            if( mprg->arglist[0] == STRING_NULL || room->mpscriptpos != 0 || atoi( mprg->arglist ) == time_info.hour )
            {
               rset_supermob( room );
               mprog_driver( mprg->comlist, supermob, NULL, NULL, NULL, TRUE );
...
void do_mpquestcomplete( CHAR_DATA * ch, char *argument )
{
   ...
   if( argument[strlen( argument ) - 1] == STRING_NULL )
   {
      argument[strlen( argument ) - 1] = STRING_NULL;
   }
...
bool CheckQuestComplete( char *quest, CHAR_DATA * player )
{
   ...
   while( fgets( read_string, 99, read_l ) != NULL )
   {
      ...
      {
         read_string[strlen( read_string ) - 1] = STRING_NULL;
      }
