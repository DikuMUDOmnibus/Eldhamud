...
void do_practice_stop ( CHAR_DATA * ch )
{

	if ( !str_cmp ( ch->in_room->area->filename, "house.are" ) && ( ch->fighting ) )
	{
		stop_fighting ( ch, TRUE );
		send_to_char( "You take a deep breath after your practice session and sit down to relax.", ch);
		do_sit( ch, "" );
	}
	else
		send_to_char( "You are not currently practicing on a dummy.", ch);
	return;
}
void do_house_assign ( CHAR_DATA * ch )
{
	...
	if ( str_cmp ( ch->in_room->area->filename, "house.are" ) )
	{
		send_to_char_color ( "&CYou must be in a house room to buy a house!!.\r\n", ch );
		return;
	}

	if ( xIS_SET ( ch->in_room->room_flags, ROOM_NOBUY ) )
	{
		send_to_char ( "You must be in a vacant house to do that.\r\n", ch );
		return;
	}
	...
	if ( homedata )
	{
		send_to_char_color ( "&CYou have a house already!!.\r\n", ch );
		return;
	}
	pMobIndex = get_mob_index ( 2001 );
	location = get_room_index ( ch->in_room->vnum );
	victim = create_mobile ( pMobIndex );
	char_to_room ( victim, location );
	do_makelocker ( ch, ch->name );
	set_house ( ch, ch->in_room->vnum, FALSE );
	give_key ( ch, ch->in_room->vnum );
	return;
}
...
void do_house ( CHAR_DATA * ch, char *argument )
{
	...
	if ( !str_cmp ( arg, "name" ) )
	{
		if ( !argument[0] || argument[0] == STRING_NULL )
		{
	...
	{
		if ( !argument || argument == STRING_NULL )
		{
	...
	if ( !str_cmp ( arg, "set" ) )
	{
		...
		if ( arg2[0] == STRING_NULL || arg3[0] == STRING_NULL || ( !is_number ( arg3 ) && str_cmp ( arg3, "addroom" ) ) )
		{
		...
		if ( !str_cmp ( arg3, "addroom" ) )
		{
			...
			if ( !argument || argument[0] == STRING_NULL || arg3[0] == STRING_NULL || !is_number ( arg3 ) )
			{
				send_to_char_color ( "&C&wHouse set <character name> addroom <current house vnum> <direction>\r\n", ch );
				return;
			}
		...
		if ( argument && argument[0] != STRING_NULL && !str_cmp ( argument, "apartment" ) )
			apartment = TRUE;
...
void do_gohome ( CHAR_DATA * ch, char *argument )
{
	...
	if ( IS_PLR_FLAG ( ch, PLR_ONMAP ) || IS_ACT_FLAG ( ch, ACT_ONMAP ) )
	{
		...
		if ( ch->mount )
		{
			...
			char_to_room ( ch->mount, get_room_index ( homedata->vnum[0] ) );
		}
		...
		send_mip_sound ( ch, "fury_action_portal.wav" );
		act ( AT_GREY, buf, ch, NULL, ch, TO_ROOM );
		do_look ( ch, " " );
		return;
	}
...
void do_accessories ( CHAR_DATA * ch, char *argument )
{
	...
	if ( arg[0] == STRING_NULL || ( is_number ( arg ) && arg2[0] == STRING_NULL ) )
	{
		...
		if ( !str_cmp ( arg, "list" ) )
		{
			...
			for ( tmpacc = first_accessory; tmpacc; tmpacc = tmpacc->next )
			{
				if ( !tmpacc->mob )
				{
					...
					{
						sprintf ( tmparg, "Accessories list: object vnum %d doesn't exist for accessory %d.", tmpacc->vnum, i );
						bug ( tmparg, 0 );
				...
				else
				{
					...
					{
						sprintf ( tmparg, "Accessories list: mob vnum %d doesn't exist for accessory %d.", tmpacc->vnum, i );
						bug ( tmparg, 0 );
...
#ifdef HOUSE_MOBS
			if ( argument && argument[0] != STRING_NULL && !str_cmp ( argument, "mob" ) )
			{
		...
		else
		{
			...
			ch_printf_color ( ch, "&C&gAverage Hp: &w%d\r\n",
			                  ( !mob->hitnodice ? ( mob->level * 8 + number_range ( mob->level * mob->level / 4,
			                                        mob->level *
			                                        mob->level ) ) : ( mob->hitnodice * number_range ( 1, mob->hitsizedice ) + mob->hitplus ) ) );
	...
	if ( !str_cmp ( arg2, "buy" ) )
	{
		...
		if ( ( tmpacc->mob && ( mobindex = get_mob_index ( tmpacc->vnum ) ) == NULL ) || ( !tmpacc->mob && ( objindex = get_obj_index ( tmpacc->vnum ) ) == NULL ) )
		{
	...
	{
		if ( !argument || argument[0] == STRING_NULL || !is_number ( argument ) )
		{
...
void do_homebuy ( CHAR_DATA * ch, char *argument )
{
	...
	if ( !is_number ( arg ) )
	{
		if ( !str_cmp ( arg, "list" ) )
		{
			...
			for ( tmpres = first_homebuy; tmpres; tmpres = tmpres->next )
			{
				...
				ch_printf_color ( ch, "&C&c%2d &C| &c%14s &C| &c%12s &C| &c%9d &C| &c%8d &C| &c%2d &CDays &c%2d &CHrs &C|\r\n",
				                  i, name, area, tmpres->bid, bidinc, days, ( ( tmpres->endtime - ( days * 48 ) ) / 2 ) );
				i++;
			}
		...
		if ( !str_cmp ( arg, "add" ) )
		{
			...
			if ( argument[0] != STRING_NULL && !str_cmp ( argument, "apartment" ) )
				apartment = TRUE;
	...
	if ( !str_cmp ( arg2, "timeremainder" ) )
	{
		...
		if ( arg3[0] == STRING_NULL || !is_number ( arg3 ) )
		{
		...
		if ( argument[0] != STRING_NULL && is_number ( argument ) )
		{
...
void do_sellhouse ( CHAR_DATA * ch, char *argument )
{
	...
	{
		if ( ( tmphome->apartment && atoi ( argument ) < MIN_APARTMENT_BID ) || ( !tmphome->apartment && atoi ( argument ) < MIN_HOUSE_BID ) )
		{
...
bool set_house ( CHAR_DATA * ch, int vnum, bool apartment )
{
	...
	if ( apartment )
		...
		location->description = STRALLOC ( "This is your desc. You can edit this with HOUSE DESC. To stop combat with your practice dummy use the STOP-PRACTICE command.\r\n" );
	...
	xSET_BIT ( location->room_flags, ROOM_NODROP );
	xSET_BIT ( location->room_flags, ROOM_LOCKER );
...
void fwrite_house ( HOME_DATA * homedata )
{
	...
	if ( !homedata || !homedata->name || homedata->name[0] == STRING_NULL )
	{
...
bool load_house_file ( char *name )
{
	...
	{
		obj_from_char ( obj );
		obj_to_room ( obj, get_room_index ( vnum ), NULL );
	}
	...
	for ( ;; )
	{
		...
		switch ( UPPER ( word[0] ) )
		{
			...
			case 'E':
				...
				if ( !str_cmp ( word, "End" ) )
				{
					if ( ( newacc->mob && ( get_mob_index ( newacc->vnum ) ) == NULL ) || ( !newacc->mob && ( get_obj_index ( newacc->vnum ) ) == NULL ) )
					{
...
void homebuy_update( )
{
	...
	for ( tmphome = first_homebuy; tmphome; tmphome = tmphomenext )
	{
		...
		if ( tmphome->bidder[0] != STRING_NULL && str_cmp ( tmphome->bidder, "None" ) && ( bidder = load_player ( tmphome->bidder ) ) != NULL )
		{
		...
		if ( tmphome->seller[0] == STRING_NULL || ( seller = load_player ( tmphome->seller ) ) == NULL )
		{
	...
	for ( ;; )
	{
		...
		switch ( UPPER ( word[0] ) )
		{
			...
			case 'E':
				...
				if ( !str_cmp ( word, "End" ) )
				{
					if ( !newmsg->name || newmsg->name[0] == STRING_NULL )
					{
...
void add_loginmsg ( char *name, short type )
{
	...
	if ( type < 0 || !name || name[0] == STRING_NULL )
		return;
