...
void do_mpstat( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf( ch, "Short description: %s.\r\nLong  description: %s", victim->short_descr, victim->long_descr[0] != STRING_NULL ? victim->long_descr : "(none).\r\n" );
...
void do_mpjunk( CHAR_DATA * ch, char *argument )
{
   ...
   if( str_cmp( arg, "all" ) && str_prefix( "all.", arg ) )
   {
      ...
      for( obj = ch->first_carrying; obj; obj = obj_next )
      {
         ...
         if( arg[3] == STRING_NULL || is_name( &arg[4], obj->name ) )
         {
...
void do_mpmload( CHAR_DATA * ch, char *argument )
{
   ...
   one_argument( argument, arg );
   if( arg[0] == STRING_NULL || !is_number( arg ) )
   {
...
void do_mp_practice( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg1[0] == STRING_NULL || arg2[0] == STRING_NULL || arg3[0] == STRING_NULL )
   {
   ...
   {
      sprintf( buf, "$n attempts to tutor you in %s, but it's beyond your comprehension.", fskill_name );
   ...
   {
      sprintf( buf, "$n shows some knowledge of %s, but yours is clearly superior.", fskill_name );
      act( AT_TELL, buf, ch, NULL, victim, TO_VICT );
      return;
   }
   /*
...
void do_mp_close_passage( CHAR_DATA * ch, char *argument )
{
   ...
   argument = one_argument( argument, arg2 );
   argument = one_argument( argument, arg3 );
   if( arg1[0] == STRING_NULL || arg2[0] == STRING_NULL || arg2[0] == STRING_NULL )
   {
...
void do_mppkset( CHAR_DATA * ch, char *argument )
{
   ...
   argument = one_argument( argument, arg );
   if( argument[0] == STRING_NULL || arg[0] == STRING_NULL )
   {
...
void do_mpquestreward( CHAR_DATA * ch, char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   if( !IS_NPC( ch ) || IS_AFFECTED( ch, AFF_CHARM ) )
   ...
   if( arg[0] == STRING_NULL )
   {
      progbug( "Mpquestreward - No argument", ch );
      return;
   }
   if( !( victim = get_char_room( ch, arg ) ) )
   {
      progbug( "Mpquestreward - victim not present!", ch );
      return;
   }
   if( IS_NPC( victim ) )
   {
      progbug( "Mobs can't have quest points!\r\n", ch );
      return;
   }
   if( argument[0] == STRING_NULL )
   {
      progbug( "Mpquestreward - no reward to grant", ch );
      return;
   }
   victim->pcdata->quest_accum += atoi( argument );
   victim->pcdata->quest_curr += atoi( argument );
}
void do_mppracticereward( CHAR_DATA * ch, char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   if( !IS_NPC( ch ) || IS_AFFECTED( ch, AFF_CHARM ) )
   ...
   if( arg[0] == STRING_NULL )
   {
      progbug( "Mppracticereward - No argument", ch );
      return;
   }
   if( !( victim = get_char_room( ch, arg ) ) )
   {
      progbug( "Mppracticereward - victim not present!", ch );
      return;
   }
   if( IS_NPC( victim ) )
   {
      progbug( "Mobs can't have practice points!\r\n", ch );
      return;
   }
   if( argument[0] == STRING_NULL )
   {
      progbug( "Mppracticereward - no reward to grant", ch );
      return;
   }
   victim->practice += atoi( argument );
}
