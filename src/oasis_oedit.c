...
ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Programs Menu                                                    EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   for( count = 0, mprg = obj->pIndexData->mudprogs; mprg; mprg = mprg->next )
   {
      ch_printf_color( d->character, "&P%2d&w) &c%s\n\r", ++count, mprog_type_to_name( mprg->type ) );
   }
   if( obj->pIndexData->mudprogs )
      send_to_char( "\n\r", d->character );
   ch_printf_color( d->character, "&PA&w) Add a new program\n\r" );
   ch_printf_color( d->character, "&PR&w) Remove an existing program\n\r" );
...
void oedit_disp_prog_choice( DESCRIPTOR_DATA * d )
{
   ...
   int i = 0;
ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Program Menu                                                     EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   if( mprg->comlist )
   {
      strcat( buf, "\n\r" );
      ...
      buf[0] = STRING_NULL;
   ch_printf_color( d->character, "&PA&w) Type: &c%s\n\r", mprog_type_to_name( mprg->type ) );
   ch_printf_color( d->character, "&PB&w) Args: &c%s\n\r", mprg->arglist );
   ch_printf_color( d->character, "&PC&w) Prog: %s\n\r", buf );
   ch_printf_color( d->character, "&PQ&w) Quit\n\r" );
...
void oedit_disp_container_flags_menu( DESCRIPTOR_DATA * d )
{
   ...
   int i;
ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Container Menu                                                   EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   for( i = 0; i < 5; i++ )
   {
...
void oedit_disp_lever_flags_menu( DESCRIPTOR_DATA * d )
{
   ...
   int counter;
ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Lever Menu                                                       EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
...
ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Layers Menu                                                      EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   send_to_char( "Choose which layer, or combination of layers fits best: \n\r\n\r", d->character );
   ch_printf_color( d->character, "[&c%s&w] &P1&w) Nothing Layers\n\r", ( obj->pIndexData->layers == 0 ) ? "X" : " " );
   ch_printf_color( d->character, "[&c%s&w] &P2&w) Silk Shirt\n\r", IS_SET( obj->pIndexData->layers, 1 ) ? "X" : " " );
   ch_printf_color( d->character, "[&c%s&w] &P3&w) Leather Vest\n\r", IS_SET( obj->pIndexData->layers, 2 ) ? "X" : " " );
   ch_printf_color( d->character, "[&c%s&w] &P4&w) Light Chainmail\n\r", IS_SET( obj->pIndexData->layers, 4 ) ? "X" : " " );
   ch_printf_color( d->character, "[&c%s&w] &P5&w) Leather Jacket\n\r", IS_SET( obj->pIndexData->layers, 8 ) ? "X" : " " );
   ch_printf_color( d->character, "[&c%s&w] &P6&w) Light Cloak\n\r", IS_SET( obj->pIndexData->layers, 16 ) ? "X" : " " );
   ch_printf_color( d->character, "[&c%s&w] &P7&w) Loose Cloak\n\r", IS_SET( obj->pIndexData->layers, 32 ) ? "X" : " " );
   ch_printf_color( d->character, "[&c%s&w] &P8&w) Cape\n\r", IS_SET( obj->pIndexData->layers, 64 ) ? "X" : " " );
   ch_printf_color( d->character, "[&c%s&w] &P9&w) Magical Effects\n\r",
...
ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ...
   if( obj->pIndexData->first_extradesc )
   {
      ...
      {
         ch_printf_color( ch, "&P%2d&w) Keyword: &O%s\n\r", ++count, ed->keyword );
      }
   }
...
ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Extra Descriptions Menu                                          EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character, "&P1&w) Keyword: &O%s\n\r", ed->keyword );
   ch_printf_color( d->character, "&P2&w) Description: \n\r&O%s&w\n\r", ed->description );
...
void oedit_disp_prompt_apply_menu( DESCRIPTOR_DATA * d )
{
   ...
   int counter = 0;
ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Apply Menu                                                       EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   for( paf = obj->pIndexData->first_affect; paf; paf = paf->next )
   {
      ch_printf_color( d->character, " &P%2d&w) ", counter++ );
      showaffect( ch, paf );
   }
   ...
   send_to_char_color( " \n\r &PA&w) Add an affect\n\r", ch );
   send_to_char_color( " &PR&w) Remove an affect\n\r", ch );
...
void oedit_liquid_type( DESCRIPTOR_DATA * d )
{
   ...
   int counter, col = 0;
   ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Liquid Type Menu                                                 EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   for( counter = 0; counter < LIQ_MAX; counter++ )
   {
      sprintf( buf, " &w%2d&P) &c%-20.20s ", counter, liq_table[counter].liq_name );
      if( ++col % 3 == 0 )
         strcat( buf, "\n\r" );
      send_to_char_color( buf, d->character );
   }
...
void oedit_disp_affect_menu( DESCRIPTOR_DATA * d )
{
   ...
   int counter, col = 0;
   ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Items Affect Menu                                                EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   for( counter = 0; counter < MAX_APPLY_TYPE; counter++ )
   {
      ...
      if( counter == 0 || counter == APPLY_EXT_AFFECT )
         continue;
      sprintf( buf, "&P%2d&w) %-20.20s ", counter, a_types[counter] );
      if( ++col % 3 == 0 )
         strcat( buf, "\n\r" );
      send_to_char_color( buf, ch );
   }
...
void oedit_disp_weapon_type_menu( DESCRIPTOR_DATA * d )
{
   char buf[MAX_STRING_LENGTH];
   CHAR_DATA *ch = d->character;
   int counter, col = 0;
   ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Weapons Type Menu                                                EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   for( counter = 0; counter < WEP_MAX; counter++ )
   {
      sprintf( buf, "&P%2d&w) %-20.20s ", counter, weapon_skills[counter] );
      if( ++col % 2 == 0 )
         strcat( buf, "\n\r" );
      send_to_char_color( buf, ch );
   }
...
void oedit_disp_damage_type_menu( DESCRIPTOR_DATA * d )
{
   ...
   OBJ_DATA *obj = d->character->dest_buf;
   ch_printf( d->character, " OLC MODE %d :  \n\r", d->olc->mode);
   ch_printf( d->character, " OLC VALUE %d :  \n\r", OEDIT_VALUE_5);
   ch_printf( d->character, " dest buffer %s :  \n\r", obj->name);
...
{
   send_to_char( "\n\rEnter the name of the spell: ", d->character );
}
/* object value 0 */
void oedit_disp_val1_menu( DESCRIPTOR_DATA * d )
{
   ...
   switch ( obj->item_type )
   {
      ...
      case ITEM_STAFF:
      case ITEM_POTION:
         send_to_char( "\n\rSpell level : ", d->character );
         break;
      case ITEM_MISSILE_WEAPON:
      case ITEM_WEAPON:
         send_to_char( "\n\rCondition : ", d->character );
         break;
      case ITEM_ARMOR:
         send_to_char( "\n\rCurrent AC : ", d->character );
         break;
         /*
          * case ITEM_QUIVER:
          * case ITEM_KEYRING: 
...
void oedit_disp_val4_menu( DESCRIPTOR_DATA * d )
{
   ...
   switch ( obj->item_type )
   {
      ...
      case ITEM_STAFF:
         ...
         break;
      case ITEM_WEAPON:
         oedit_disp_weapon_type_menu( d ); //weapon skill type
         break;
      case ITEM_DRINK_CON:
      case ITEM_FOUNTAIN:
      case ITEM_FOOD:
...
void oedit_disp_type_menu( DESCRIPTOR_DATA * d )
{
   ...
   int counter, col = 0;
   ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Item Type Menu                                                   EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   for( counter = 0; counter < MAX_ITEM_TYPE; counter++ )
   {
      sprintf( buf, "&P%2d&w) %-20.20s ", counter, o_types[counter] );
      if( ++col % 3 == 0 )
         strcat( buf, "\n\r" );
      send_to_char_color( buf, d->character );
   }
...
void oedit_disp_extra_menu( DESCRIPTOR_DATA * d )
{
   ...
   int counter, col = 0;
   ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Extra Flags Menu                                                 EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   for( counter = 0; counter < MAX_ITEM_FLAG; counter++ )
   {
      sprintf( buf, "&P%2d&w) %-20.20s ", counter + 1, capitalize( o_flags[counter] ) );
      if( ++col % 2 == 0 )
         strcat( buf, "\n\r" );
      send_to_char_color( buf, d->character );
   }
...
void oedit_disp_wear_menu( DESCRIPTOR_DATA * d )
{
   ...
   int counter, col = 0;
   ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Wear Flags Menu                                                  EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   for( counter = 0; counter <= ITEM_WEAR_MAX; counter++ )
   {
      if( 1 << counter == ITEM_DUAL_WIELD )
         continue;
      sprintf( buf, "&P%2d&w) %-20.20s ", counter + 1, capitalize( w_flags[counter] ) );
      if( ++col % 2 == 0 )
         strcat( buf, "\n\r" );
      send_to_char_color( buf, d->character );
   }
   sprintf( buf, "\n\rWear flags: &c%s&w\n\rEnter wear flag, 0 to quit:  ", flag_string( obj->wear_flags, w_flags ) );
   send_to_char_color( buf, d->character );
   return;
}
/* display main menu */
void oedit_disp_menu( DESCRIPTOR_DATA * d )
{
   ...
   char buf[MAX_STRING_LENGTH];
   ch_printf_color( d->character,"&D\n\r--------------------------------------------------------------------------------\n\r");
   ch_printf_color( d->character,"&c Objects Main Menu                                                EldhaMUD OLC\n\r" );
   ch_printf_color( d->character,"&D--------------------------------------------------------------------------------\n\r");
   /*
    ...
    */
	sprintf( buf,   "&P7&w) Wear flags  : &c%s\n\r" 
			"&P8&w) Weight      : &c%d\n\r" 
			"&P9&w) Cost        : &c%d\n\r" 
			"&PA&w) Rent(unused): &c%d\n\r" 
			"&PB&w) Timer       : &c%d\n\r" 
			"&PC&w) Level       : &c%d\n\r"  /* -- Object level . */
			"&PD&w) Layers      : &c%d\n\r"
			"&PE&w) Values      : &c%d %d %d %d %d %d\n\r"
			"&PF&w) Affect menu\n\r"
			"&PG&w) Extra descriptions menu\n\r"
			"&PQ&w) Quit\n\r"
			"Enter choice : \n\r",
...
void oedit_parse( DESCRIPTOR_DATA * d, char *arg )
{
   ...
   switch ( OLC_MODE( d ) )
   {
      ...
      case OEDIT_MAIN_MENU:
         ...
         switch ( UPPER( arg[0] ) )
         {
            ...
            case '7':
               ...
               break;
            case '8':
               send_to_char( "\n\rEnter weight : ", d->character );
               OLC_MODE( d ) = OEDIT_WEIGHT;
               break;
            case '9':
               send_to_char( "\n\rEnter cost : ", d->character );
               OLC_MODE( d ) = OEDIT_COST;
               break;
            case 'A':
               send_to_char( "\n\rEnter cost per day : ", d->character );
               OLC_MODE( d ) = OEDIT_COSTPERDAY;
               break;
            case 'B':
               send_to_char( "\n\rEnter timer : ", d->character );
            ...
            case 'D':
               if( IS_SET( obj->wear_flags, ITEM_WEAR_BODY )
                   ...
                   || IS_SET( obj->wear_flags, ITEM_WEAR_HANDS ) || IS_SET( obj->wear_flags, ITEM_WEAR_LEGS ) )
               {
      ...
      case OEDIT_EXTRAS:
         while( arg[0] != STRING_NULL )
         {
            ...
            else
            {
               ...
               olc_log( d, "%s the flag %s", xIS_SET( obj->extra_flags, number ) ? "Added" : "Removed", o_flags[number] );
            }
            /*
         ...
         if( is_number( arg ) )
         {
            ...
            else
            {
               ...
               olc_log( d, "%s the wearloc %s",
                        IS_SET( obj->wear_flags, 1 << number ) ? "Added" : "Removed", w_flags[number] );
            }
         }
         else
         {
      ...
      case OEDIT_VALUE_4:
         ...
         return;
      case OEDIT_VALUE_5:
         ch_printf_color( d->character,"entered case VALUE5\n\r");
         number = atoi( arg );
         switch ( obj->item_type )
         {
            case ITEM_SALVE:
      ...
      case OEDIT_MPROGS_TYPE:
         ...
         olc_log( d, "Changed prog %s to %s",
                  d->character->tempnum > 0 ? mprog_type_to_name( d->character->tempnum ) : "(none)",
                  mprog_type_to_name( number ) );
         oedit_disp_prog_choice( d );
         return;
