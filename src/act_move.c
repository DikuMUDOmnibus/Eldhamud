...
char *grab_word ( char *argument, char *arg_first )
{
	...
	if ( *argument == '\'' || *argument == '"' )
		cEnd = *argument++;
	while ( *argument != STRING_NULL || ++count >= 255 )
	{
		if ( *argument == cEnd )
		{
			argument++;
...
ch_ret move_char ( CHAR_DATA * ch, EXIT_DATA * pexit, int fall, int direction )
{
	...
	if ( IS_PLR_FLAG ( ch, PLR_ONMAP ) || IS_ACT_FLAG ( ch, ACT_ONMAP ) )
	{
		...
		if ( newx == ch->x && newy == ch->y )
			return rNONE;
		if ( ( IS_NPC ( ch ) && get_terrain ( ch->map, newx, newy ) == SECT_EXIT ) || ( IS_NPC ( ch ) && get_terrain ( ch->map, newx, newy ) == SECT_CITY ) )
			return rNONE;
		...
		if ( IS_SET ( pexit->exit_info, EX_CLIMB ) )
		{
			...
			if ( !found && !ch->mount )
			{
				if ( ( !IS_NPC ( ch ) && number_percent( ) > LEARNED ( ch, gsn_climb ) ) )
				{
					...
					{
						retcode = move_char ( ch, pexit, 1, direction );
						return retcode;
					}
...
void do_open ( CHAR_DATA * ch, char *argument )
{
	...
	if ( ( pexit = find_door ( ch, arg, TRUE ) ) != NULL )
	{
		...
		if ( !IS_SET ( pexit->exit_info, EX_SECRET ) || ( pexit->keyword && nifty_is_name ( arg, pexit->keyword ) ) )
		{
			...
			send_mip_sound ( ch, "fury_action_door_open.wav" );
...
void do_close ( CHAR_DATA * ch, char *argument )
{
	...
	if ( ( pexit = find_door ( ch, arg, TRUE ) ) != NULL )
	{
		...
		send_mip_sound ( ch, "fury_action_door_close.wav" );
		/*
...
void do_unlock ( CHAR_DATA * ch, char *argument )
{
	...
	if ( ( pexit = find_door ( ch, arg, TRUE ) ) != NULL )
	{
		...
		{
			send_to_char ( "*Click*\r\n", ch );
			send_mip_sound ( ch, "fury_action_door_unlock.wav" );
			count = key->count;
			key->count = 1;
...
void do_stand ( CHAR_DATA * ch, char *argument )
{
	...
	send_mip_sound ( ch, "fury_action_wake.wav" );
	return;
}
...
void do_sleep ( CHAR_DATA * ch, char *argument )
{
	...
	send_mip_sound ( ch, "fury_action_snore.wav" );
...
void do_enter ( CHAR_DATA * ch, char *argument )
{
	...
	if ( argument[0] == STRING_NULL )
	{
		for ( pexit = ch->in_room->first_exit; pexit; pexit = pexit->next )
			...
			{
				move_char ( ch, pexit, 0, DIR_SOMEWHERE );
				return;
			}
		if ( ch->in_room->sector_type != SECT_INSIDE && xIS_OUTSIDE ( ch ) )
			for ( pexit = ch->in_room->first_exit; pexit; pexit = pexit->next )
