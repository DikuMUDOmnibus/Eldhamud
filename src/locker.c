...
#define LOCKER_PRICE    50000
#define LOCKER_CAPACITY 5000
LOCKER_DATA *load_locker( CHAR_DATA * ch )
{
   LOCKER_DATA *locker;
   OBJ_DATA *obj;
...
void fwrite_locker( CHAR_DATA * ch )
{
   ...
   if( ( fp = fopen( strsave, "w" ) ) != NULL )
   {
      ...
      for( obj = ch->in_room->first_content; obj; obj = obj->next_content )
      {
         fwrite_obj( ch, obj, fp, 0, OS_LOCKER, TRUE );
      }
      fprintf( fp, "#END\n" );
      fclose( fp );
   }
...
void do_locker( CHAR_DATA * ch, char *argument )
{
   ...
   else if( !str_cmp( arg, "close" ) )
   {
      ...
      {
         sprintf( buf, "The weight limit of your locker has beed exceeded.  " "You must carry\r\nsome items out when you leave.\r\n" );
         send_to_char( buf, ch );
         return;
      }
      /*
