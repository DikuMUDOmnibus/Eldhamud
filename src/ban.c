...
void do_ban( CHAR_DATA * ch, char *argument )
{
    ...
    */
   if( arg4[0] != STRING_NULL && is_number( arg4 ) )
      btime = atoi( arg4 );
   else
      btime = -1;
   /*
   ...
   else if( !str_cmp( arg1, "show" ) )
   {
      ...
      if( !str_cmp( arg2, "site" ) )
      {
         ...
         if( temp[strlen( temp ) - 1] == '*' )
            temp[strlen( temp ) - 1] = STRING_NULL;
      }
      else
         goto syntax_message;
...
int add_ban( CHAR_DATA * ch, char *arg1, char *arg2, int btime, int type )
{
   ...
   switch ( ch->substate )
   {
      ...
      case SUB_NONE:
      {
         ...
         if( arg[0] == STRING_NULL || arg2[0] == STRING_NULL )
            return 0;
         if( is_number( arg2 ) )
         {
            level = atoi( arg2 );
         ...
         switch ( type )
         {
            case BAN_SITE:
            {
               ...
               {
                  suffix = TRUE;
                  name[strlen( name ) - 1] = STRING_NULL;
               }
      ...
      case BAN_SITE:
         ...
         for( bnum = 1; pban; pban = pban->next, bnum++ )
         {
            pager_printf( ch, "[%2d] %-4s (%2d) %-24s %-15s %4d  %c%s%c\r\n",
                          ...
                          pban->ban_time, pban->ban_by, pban->duration, ( pban->prefix ) ? '*' : ' ', pban->name, ( pban->suffix ) ? '*' : ' ' );
         }
         return;
      default:
